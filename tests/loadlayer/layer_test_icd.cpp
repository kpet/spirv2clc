...
std::optional<std::string> get_environment(const std::string& variable)
{
  ...
  if(var_size == 0 || err != 0) return std::nullopt;
...
_cl_platform_id::_cl_platform_id()
  ...
  , vendor{ "spirv2clc" }
  , profile{ "FULL_PROFILE" }
  , name{ "SPIRV2CLC Layer Test ICD" }
  , extensions{ "cl_khr_icd" }
  , suffix{ "spv2clc" }
{
  init_dispatch();

  if (auto ICD_VERSION = get_environment("SPIRV2CLC_ICD_VERSION"); ICD_VERSION)
  {
  ...
  if (auto ICD_SUPPORT_SPIRV = get_environment("SPIRV2CLC_ICD_SUPPORT_SPIRV"); ICD_SUPPORT_SPIRV)
  {
